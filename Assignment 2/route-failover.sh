...
LOG_FILE="./log/route_results.log"
ITERATIONS=3
TIMEOUT=30
...
change_route_priority() {
    ...
    echo "$(get_timestamp) - Recreated route $route_name with priority $priority to $next_hop_instance" | tee -a "$LOG_FILE"
}
...
main() {
    ...
    echo "=== ROUTE MODIFICATION FAILOVER TEST START ===" | tee -a "$LOG_FILE"

    change_route_priority "$VM2_ROUTE" 100 "$ACTIVE_VM"
    change_route_priority "$VM3_ROUTE" 200 "lbtcp-vm3"
    ...
    for i in $(seq 1 $ITERATIONS); do
        ...
        change_route_priority "$VM2_ROUTE" 200 "$ACTIVE_VM"
        change_route_priority "$VM3_ROUTE" 100 "lbtcp-vm3"
    ...
    echo "\n=== ROUTE MODIFICATION FAILOVER TEST COMPLETE ===" | tee -a "$LOG_FILE"
